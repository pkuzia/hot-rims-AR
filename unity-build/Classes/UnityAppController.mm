...
- (void)initUnity {
    ...
    _closeButton.frame = CGRectMake(10, 10, 60, 30);
    [_closeButton setTitle:NSLocalizedString(@"unityCloseButton", "") forState:UIControlStateNormal];
    [_closeButton setTintColor:[UIColor whiteColor]];
    [_closeButton addTarget:self action:@selector(closeUnity) forControlEvents:UIControlEventTouchUpInside];
    
    [self.rootViewController.view addSubview:_closeButton];
}
- (void)closeUnity {
    [self closeUnityWithCompletion:nil];
}
- (void)closeUnityWithCompletion: (void (^ __nullable)(void))completion {
